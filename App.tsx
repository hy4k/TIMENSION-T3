...
import { generateDailyHeadline, chatWithMentor, hasGlobalApiKey, simulateAlternateHistory, generateVintageMap, generateLocationTrivia, generateHistoricalPhotos, generateImage, testApiKey } from './services/geminiService';
...
const LoginView: React.FC<{ onLogin: () => void }> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-paper flex flex-col items-center py-8 px-4 relative z-50">
       {/* Newspaper Header */}
                ...
                <div className="space-y-5">
                    ...
                    <div onClick={handleApiKeySelect} className={`border-2 border-ink p-2 cursor-pointer hover:bg-ink/5 text-center ${apiKeySet ? 'opacity-70' : 'animate-pulse bg-alert-red/10'}`}>
                        ...
                        <div className={`font-bold text-xs uppercase ${apiKeySet ? 'text-ink' : 'text-alert-red'}`}>
                            {apiKeySet ? (isGlobalKey ? '[ SYSTEM ONLINE ]' : '[ KEY INSERTED ]') : '[ SIGNAL LOST - CLICK TO FIX ]'}
                        </div>
                    </div>
